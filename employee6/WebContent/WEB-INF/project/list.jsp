...
<div class="col-sm-3">
<input type="text"  class="form-control" name="name" placeholder="请输入项目名" value=${c.name} >
</div>
...
<button  type="submit" class="btn btn-primary">搜索</button>
	...
	<tbody>
		...
		<ul class="pagination">
		<li id="first" ><a href="pro?ye=1&name=${c.name}" >首页</a></li>
    <li id="pre" ><a href="pro?ye=${p.ye-1}&name=${c.name}" >上一页</a></li>
   <c:forEach begin="${p.beginYe }" end="${p.endYe }" varStatus="status">
    <li <c:if test="${p.ye==status.index}"> class="active"</c:if>><a href="pro?ye=${status.index}&name=${c.name}">${status.index}</a></li>
    </c:forEach>
    <li id="next"><a href="pro?ye=${p.ye+1}&name=${c.name}">下一页</a></li>
    <li id="last" ><a href="pro?ye=${p.maxYe}&name=${c.name}" >尾页</a></li>
</ul>
<div>
